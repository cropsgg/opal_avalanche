...
export default function ChatPage() {
  ...
  const handleSendMessage = async (
    ...
    // Removed unnecessary matterId check as it's not defined or used
    ...
    try {
      ...
      if (llmResponse) {
        ...
        const assistantMessage: Message = {
          ...
          explainability: isFirstResponse
            ? llmResponse.explainability
            : undefined,
  ...
  const fetchLLMResponse = async (
    ...
    try {
      ...
      if (data && data.final_verdict) {
        const composedContent = `Verdict: ${data.final_verdict.replaceAll(
          "_",
          " "
        )} (confidence ${(data.final_confidence * 100).toFixed(1)}%).\n\n${
          data.explanation?.conclusion ?? ""
        }`.trim();
        const composedExplainability = `The DAO ensemble aggregated agent votes using confidence × weight. Winning verdict '${
          data.final_verdict
        }' achieved ${(data.final_confidence * 100).toFixed(1)}% confidence.`;
        return {
  ...
  return (
    <div
      className={`${poppins.variable} ${inter.variable} min-h-screen bg-white text-black`}
    >
      {/* Header */}
      <ChatHeader />
